...
from .observer.build import build_observer
from .quantizer.build import build_quantizer
...
class QConv2d(nn.Conv2d):
    def __init__(self,
        ...
        self.quantizer = build_quantizer(self.quantizer_str, self.bit_type,
                                         self.observer, self.module_type, self.use_fisher_hessian)
    ...
    def forward(self, x):
        ...
        print("The quantization interval is updated using the `use_fisher_hessian` information of the uniform at the Conv2d layer.")
...
class QIntSoftmax(nn.Module):
    ...
    def forward(self, x, scale):
        if self.log_i_softmax and scale is not None:
            ...
            softmax_out = torch.round(exp_int_sum / exp_int)  # This involves approximations for the exponential function exp.
