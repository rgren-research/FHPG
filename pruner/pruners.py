...
class BaseUnitPruner(object):
    ...
    """
    def __init__(self, model, pruning_parameters_list, pruning_settings, head_mask, neuron_mask, logger, scorer=taylor1Scorer):
        ...
        self.head_mask = head_mask  # The optimal masks for MHA and FFN obtained from the previous pruning process also guide the pruning.
    ...
    # head_mask、 neuron_mask fusion...
    from transformers import ViTConfig
    from transformers import ViTModel
    config = ViTConfig.from_pretrained("/retraining-free-pruning-main/deit-base")
    model = ViTModel.from_pretrained("/retraining-free-pruning-main/deit-base", config=config)
    def enforce_pruning(self, head_mask=None, neuron_mask=None):
        # head_mask applied to MHA
        if self.head_mask is not None:
            for i, layer in enumerate(self.masked_model.get_attention_layers()):
                layer.apply_head_mask(self.head_mask[i])
        # neuron_mask applied to FFN
        if self.neuron_mask is not None:
            for i, layer in enumerate(self.masked_model.get_ffn_layers()):  # Similarly, let's assume a method to retrieve all FFN layers.
                layer.apply_neuron_mask(self.neuron_mask[i])
