...
def search_optimal_mask(
    ...
    #
    # This function measures the importance of the head by calculating a set of head gradient Fish information.
    #
    head_importance = compute_fisher_info(head_grads)
    neuron_importance = compute_fisher_info(neuron_grads)
    head_importance1 = compute_fisher_info_taylor1(head_grads)
    neuron_importance1 = compute_fisher_info_taylor1(neuron_grads)
    # Print Shape
    print("Shape of Fisher Information neuron:", neuron_importance.shape)
    # Print content
    print("Fisher Information neuron values :", neuron_importance)

    alpha = 0.7  # Assume the weight of the original Fisher information is 0.7, and the weight of the adjusted Fisher information is 0.3.
    combined_head_importance = alpha * head_importance + (1 - alpha) * head_importance1
    combined_neuron_importance = alpha * neuron_importance + (1 - alpha) * neuron_importance1
    # Subsequent sorting and mask generation operations are performed based on combined_head_importance and combined_neuron_importance.
    print("The fusion process ends after calculating the Fisher information using fisher_info and taylor1.")
    ...
    sorted_head_importance, sorted_head_indicies = combined_head_importance.view(-1).sort(descending=True)
    sorted_neuron_importance, sorted_neuron_indicies = combined_neuron_importance.view(-1).sort(descending=True)
