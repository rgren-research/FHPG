...
from utils import CPUTimer, GPUTimer
...
def mha_lut(config, device, input_shape, num_warmup=100, num_iter=100):
    ...
    for num_heads in range(1, num_attention_heads + 1):
        model = ViTMHA(num_heads, attention_head_size, hidden_size)
...
def ffn_lut(config, device, input_shape, num_warmup=10, num_iter=10):
    ...
    for num_neurons in range(1, intermediate_size + 1):
        model = ViTFFN(hidden_size, num_neurons)
        model = model.to(device).eval()
...
# parameter： --model_ckpt_dir /deit-base --bs 16  --output_dir /FHPG/FHPG-main/output
if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("--model_ckpt_dir", type=str, required=True)
    parser.add_argument("--device", choices=["cpu", "cuda"], default="cuda")
    parser.add_argument("--bs", type=int, required=True, help="Batch size for latency calculations.")
    parser.add_argument("--output_dir", type=str, required=True, help="Directory to save latency look-up tables.")
    args = parser.parse_args()

    os.makedirs(args.output_dir, exist_ok=True)
    config = ViTConfig.from_pretrained(args.model_ckpt_dir)

    seq_len = int(config.image_size / config.patch_size) ** 2
    input_shape = (args.bs, seq_len, config.hidden_size)
